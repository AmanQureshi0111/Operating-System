  ...
  static void run(const vector<Process> &processes) {
    ...
    while (done < n) {
      ...
      for (int i = 0; i < n; i++) {
        if (!completed[i] && processes[i].arrival <= current_time) {
          if (processes[i].priority < best_priority) {
            ...
            selected = i;
          } else if (processes[i].priority == best_priority && selected != -1 &&
      ...
      waiting_time[selected] =
          turnaround_time[selected] - processes[selected].burst;
    ...
    cout << left << setw(10) << "PID" << setw(10) << "AT" << setw(10) << "BT"
         << setw(10) << "PR" << setw(10) << "WT" << setw(10) << "TAT" << '\n';
    cout << "-------------------------------------------------------\n";
    ...
    for (int i = 0; i < n; i++) {
      ...
      cout << left << setw(10) << ("P" + to_string(processes[i].id)) << setw(10)
           ...
           << processes[i].priority << setw(10) << waiting_time[i] << setw(10)
           << turnaround_time[i] << '\n';
    }
